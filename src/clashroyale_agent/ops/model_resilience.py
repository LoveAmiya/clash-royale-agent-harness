...
class ModelProviderGuard:
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            ...
            return {
                ...
                "stream_modes": {
                    mode: int(self._stream_modes[mode])
                    for mode in ("streaming", "fallback_chunked", "unavailable")
                },
            }
    ...
    def render_prometheus(self) -> str:
        ...
        for (operation, outcome), value in sorted(calls.items()):
            lines.append(
                'cr_agent_model_provider_calls_total{'
                f'provider="{self.provider_id}",operation="{operation}",outcome="{outcome}"'
                f"}} {value}"
            )
        for mode, value in sorted(stream_modes.items()):
            lines.append(
                f'cr_agent_model_stream_quality_total{{provider="{self.provider_id}",mode="{mode}"}} {value}'
            )
        capability_value = {"unknown": 0, "unsupported": 0, "supported": 1}.get(
            streaming_capability, 0
        )
        lines.append(
            f'cr_agent_model_stream_capability{{provider="{self.provider_id}",status="{streaming_capability}"}} '
            f"{capability_value}"
        )
        lines.append(
