...
# Keep request pacing at the API-client layer, but allow a tiny fetch pipeline
# so network latency does not leave the single permitted request slot idle.
SUPERCELL_FETCH_CONCURRENCY = _bounded_int("SUPERCELL_FETCH_CONCURRENCY", 2, 1, 4)
...
# The default keeps the request stream conservative while avoiding the previous
# single-slot bottleneck; 429 responses still trigger API-client cooldowns.
SUPERCELL_HIGH_VOLUME_REQUESTS_PER_SECOND = max(
    0.1,
    min(float(os.getenv("SUPERCELL_HIGH_VOLUME_REQUESTS_PER_SECOND", "2")), 4.0),
)
...
__all__ = [
    name
    for name in globals()
    if name.isupper() or name in {"DATA_DIR", "CARD_ALIAS_FILE", "RAG_DOCS_FILE"}
]
